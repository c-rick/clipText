		...
		var main = {
			...
			init: function init(selectors, opts) {
				...
				selectEl.forEach(function (i) {
					...
					var animation = _this.defalutOpt.animation;
					animation = animation ? typeof animation === 'number' ? animation : 60 : animation;
					if ($i && $i.forEach) {
						$i.forEach(function (j) {
							cacheEl.push({ el: j, orText: j.innerText });
							_this.clip(j);
							animation && _this.setAnimation(j, animation);
						});
					} else {
						cacheEl.push({ el: $i, orText: $i.innerText });
						_this.clip($i);
						animation && _this.setAnimation($i, animation);
					}
				});
				// init resize
				var resizeCB = function resizeCB(delay) {
					var timeout = null;
					var cb = function cb() {
						cacheEl.forEach(function (i) {
							_this.clip(i.el, _this.defalutOpt, i.orText);
						});
						timeout = null;
					};
					return function () {
			...
			countClip: function countClip(str) {
				...
				var finalLen = clipText === '...' ? 1 : otherText ? Math.ceil(otherText.length / 2) + (clipLen - otherText.length) : clipLen;
			...
			chromeClip: function chromeClip($el, opt) {
				...
				this.setStyle($el, ['overflow: hidden', 'display: -webkit-box', '-webkit-box-orient: vertical', '-webkit-line-clamp: ' + this.defalutOpt.line, 'text-align: left']);
			},
			setAnimation: function setAnimation($el) {
				...
				if ($el.children.length) {
					...
					childrenText = function childrenText() {
						...
						_this2.setAnimation(child, time);
					};
				}
